...
foreach ($objORM->FetchAll("Enabled = 1 GROUP BY  Name", 'Name,IdKey,LocalName', TableIWNewMenu) as $objPGander) {
    $strSelected = '';
    if (@$_GET['PGender'] == $objPGander->Name and isset($_GET['PGender']))
        $strSelected = 'selected';
    $strPGender .= '<option ' . $strSelected . ' value="' . $objPGander->Name . '" >' . $objPGander->LocalName . '</option>';
}
