...
foreach ($objORM->FetchAll("ProductType != '' GROUP BY  ProductType", 'ProductType', TableIWLocalProducts) as $objProductType) {
    $strSelected = '';
...
foreach ($objORM->FetchAll("BrandName != '' GROUP BY  BrandName", 'BrandName', TableIWLocalProducts) as $objBrandName) {
    $strSelected = '';
...
$intTotalFind = 0;
$intTotalFind = $objORM->DataCount($SCondition,TableIWLocalProducts);
foreach ($objORM->FetchLimit($SCondition, 'ModifyDate,Content,IdKey,ProductId,Name,PGender,PCategory,PGroup,PGroup2,Attribute,ProductType,BrandName,MainPrice,WeightIdKey,ModifyId,Enabled,IdRow,ApiContent', 'IdRow DESC', $strLimit, TableIWLocalProducts) as $ListItem) {
    ...
    $objORM->DataUpdate($UCondition, $USet, TableIWLocalProducts);
    ...
    $strListBody .= (new ListTools())->TableBody($ListItem, $ToolsIcons, 14, $objGlobalVar->en2Base64($ListItem->IdKey . '::==::' . TableIWLocalProducts, 0));
}
