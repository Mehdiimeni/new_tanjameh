...
class TimeTools extends Regularization
{
    public function TimetoStr($strTime)
    {
        return strtotime($strTime);
    }

    public function TimetoStrNow()
    {
        return strtotime("now");
    }

    public function getDateTime($strTime)
    {
        return parent::JsonEncode(new DateTime($strTime));
    }

    public function getDateTimeNow()
    {

        return parent::JsonEncode(new DateTime("now"));
    }

    public function getDateTimeOther($intDiff)
    {
        $strNowDate = new DateTime("now");
        return parent::JsonEncode($strNowDate->modify($intDiff . ' day'));
    }

    public function DateTimeDiff($strTime1, $strTime2)
    {
        $date1 = new DateTime($strTime1);
        $date2 = new DateTime($strTime2);
        $interval = $date1->diff($date2);
        return array("years" => $interval->y, "months" => $interval->m, "days" => $interval->d, "hours" => $interval->h, "minutes" => $interval->i, "seconds" => $interval->s);
    }

    public function DateTimeNowDiff($strTime)
    {
        $date1 = new DateTime($strTime);
        $date2 = new DateTime("now");
        $interval = $date1->diff($date2);
        return array("years" => $interval->y, "months" => $interval->m, "days" => $interval->d, "hours" => $interval->h, "minutes" => $interval->i, "seconds" => $interval->s);
    }

    public function DateDayStepper($strCountDay)
    {
        $dateTime = new DateTime("now");
        return $dateTime->modify($strCountDay.' day');
    }

    public function DateTimeToShamsi($strTime)
    {
        $strDate = new DateTime($strTime);
        return $this->gregorian_to_jalali($strDate->y, $strDate->m, $strDate->d);
    ...
    public function jdate($format, $timestamp = '', $none = '', $time_zone = 'Asia/Tehran', $tr_num = 'fa')
    {
        ...
        $ts = $T_sec + (($timestamp == ''
                or $timestamp == 'now') ? time() : $this->tr_num($timestamp));
